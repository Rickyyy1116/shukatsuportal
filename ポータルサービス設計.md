...
### 9-3. デプロイ基盤実装状況（2026/02/23時点）

| 項目 | 状態 | 実装内容 |
|------|------|---------|
| OpenNext設定 | ✅ 完了 | `open-next.config.ts` を追加し `npm run build:worker` 成功 |
| Workers設定 | ✅ 完了 | `wrangler.jsonc` 追加（`name: shukatsuportal`, `main: .open-next/worker.js`） |
| GitHub Actions | ✅ 完了 | `.github/workflows/deploy-cloudflare-workers.yml` で `main` push時に自動デプロイ |
| GitHub Pages | ✅ 整理済み | `deploy-pages.yml` を削除し、Workers運用へ一本化 |
| デプロイ実行 | ⚠️ 未完了 | Cloudflare Secrets 未設定のため本番反映待ち（`CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`） |
